...
class SoASchema:
    """Schema for a struct-of-arrays batch.

    Each field maps to a numpy dtype string (e.g. 'int32', 'float32').
    """
...
class SoABatchHandle:
    """A transferable handle to a batch: one handle per column."""
...
class SoABatch:
    """Convenience wrapper that can allocate/open SoA batches."""
    ...
    def open(arena: SharedMemoryArena, batch: SoABatchHandle) -> Tuple[Dict[str, Any], Any]:
        """Open zero-copy numpy arrays for each column.

        Returns (columns_dict, shm_keeper).

        `shm_keeper` is an object you must keep alive and close when done.
        Currently it's a dict of SharedMemory objects.
        """
        import numpy as np  # type: ignore
