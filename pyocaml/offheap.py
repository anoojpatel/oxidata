...
class OffHeapScope:
    """Convenience context that owns both a `Scope` and a `SharedMemoryArena`.

    Allocate payloads off-heap into shared memory, and return lifetime-managed
    `Owned[Handle]` capabilities.

    When the context exits:
    - all Owned values are invalidated (Scope semantics)
    - the underlying shared memory segment is closed+unlinked

    This is the primary "pythonic, refcount-light" surface:
    - keep payloads as off-heap bytes/ndarrays
    - pass handles between threads/processes
    - only materialize Python objects on demand
    """
...
def borrow_region(owned_handle: Owned[Handle]) -> Iterator[OpenedRegion]:
    """Borrow an off-heap region as a zero-copy memoryview.

    The borrow enforces:
    - lifetime (Scope must be alive)
    - aliasing discipline (borrow vs borrow_mut)

    Returns a memoryview plus a SharedMemory handle that is closed on exit.

    Note: this is for read-only semantics at the Python level. For mutation,
    use `borrow_region_mut`.
    """
...
def borrow_region_mut(owned_handle: Owned[Handle]) -> Iterator[OpenedRegion]:
    """Mutable borrow of a region.

    This is still just a memoryview; the key is that the borrow rules enforce
    single-writer at the capability layer.
    """
...
class OffHeap(Generic[T]):
    """A typed capability wrapper around `Owned[Handle]`.

    `freeze()` makes the underlying capability immutable (no future mutable borrows).
    """
...
class OffHeapArray(OffHeap[Any]):
    """A numpy ndarray stored off-heap."""
